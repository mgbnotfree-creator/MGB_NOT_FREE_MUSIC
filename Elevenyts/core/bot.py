...
class Bot(pyrogram.Client):
    ...
    async def boot(self) -> None:
        ...
        try:
            ...
            raise SystemExit(
                f"❌ Bot could not access the logger group: {self.logger}\n"
                f"Error: {ex}\n"
                f"Make sure to add the bot to the logger group."
            )

        # Verify admin status
        if member.status != pyrogram.enums.ChatMemberStatus.ADMINISTRATOR:
            raise SystemExit(
                f"❌ Bot is not an admin in logger group: {self.logger}\n"
                f"Please promote the bot to admin."
            )
