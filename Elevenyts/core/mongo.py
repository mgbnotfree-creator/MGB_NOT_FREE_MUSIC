...
class MongoDB:
    ...
    async def load_cache(self) -> None:
        """Load essential settings into memory."""
        # Note: You may want to define load_cache if you rely on it above
        await self.get_logger()
        await self.get_maintenance()
        await self.get_gbanned()
    ...
    # SUDO METHODS (If applicable from other files)
    async def get_sudoers(self) -> list[int]:
        doc = await self.cache.find_one({"_id": "sudoers"})
        return doc.get("user_ids", []) if doc else []

    async def add_sudoer(self, user_id: int) -> None:
        await self.cache.update_one(
            {"_id": "sudoers"},
            {"$addToSet": {"user_ids": user_id}},
            upsert=True
    ...
    async def del_sudoer(self, user_id: int) -> None:
        await self.cache.update_one(
            {"_id": "sudoers"},
            {"$pull": {"user_ids": user_id}}
        )
