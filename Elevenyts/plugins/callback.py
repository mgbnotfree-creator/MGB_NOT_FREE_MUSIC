...
async def _controls(_, query: types.CallbackQuery):
    ...
    if not has_permission:
        return await query.answer("🚫 You don't have permission to use this.", show_alert=True)
...
async def handle_seek(query: types.CallbackQuery, chat_id: int, action: str, user: str):
    ...
    if not media or media.is_live:
        return await query.answer("🚫 Cannot seek in live streams!", show_alert=True)
    
    if not media.duration_sec or media.duration_sec == 0:
        return await query.answer("🚫 Cannot seek in this track!", show_alert=True)
    ...
    if action == "seek_back_10":
        ...
        return await query.answer("🚫 Invalid seek action!", show_alert=True)
    
    current_time = getattr(media, 'time', 0)
    new_time = max(0, min(current_time + seconds, media.duration_sec - 5))
    
    if new_time == 0 and seconds < 0:
        return await query.answer("⏮️ Already at the beginning!", show_alert=True)
    if new_time >= media.duration_sec - 5 and seconds > 0:
        return await query.answer("⏭️ Too close to the end!", show_alert=True)
...
async def handle_shuffle(query: types.CallbackQuery, chat_id: int, user: str):
    ...
    if not items or len(items) <= 1:
        return await query.answer("🚫 Queue is empty or has only one track!", show_alert=True)
    
    current = items[0] if items else None
    remaining = items[1:] if len(items) > 1 else []
    
    if not remaining:
        return await query.answer("🚫 No tracks to shuffle!", show_alert=True)
...
async def handle_autoplay(query, chat_id: int, user: str):
    ...
    status = "ℹ️ ON" if new_state else "ℹ️ OFF"
...
async def _setlang(_, query: types.CallbackQuery):
    ...
    if code not in _LANG_NAMES:
        return await query.answer("⚠️ Invalid language.", show_alert=True)
