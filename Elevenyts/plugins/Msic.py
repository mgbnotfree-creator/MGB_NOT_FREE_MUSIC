...
async def _maintenance_mode_check(_, m: types.Message):
    ...
    if maintenance:
        ...
        try:
            await m.reply_text(
                "<blockquote><b>🛠️ Bot Under Maintenance</b>\n\n"
    ...
    while True:
        try:
            ...
            for ub in userbot.clients:
                left = 0
                try:
                    async for dialog in await ub.get_dialogs():
                    ...
                    logger.error(f"Auto-leave error for assistant {getattr(ub.me, 'username' if hasattr(ub, 'me') and ub.me else 'Unknown')}: {e}")
                    continue
        except Exception as e:
        ...
        while True:
            try:
                ...
                if remaining <= 30:
                    ...
                    if next_item and not next_item.file_path:
                        asyncio.create_task(_preload_next(chat_id, next_item))
